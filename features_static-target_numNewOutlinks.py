...
"""
To ensure a large minimum number of executors:

	time spark-submit --master yarn --deploy-mode cluster --executor-cores 2 --conf spark.dynamicAllocation.minExecutors=25 --conf spark.dynamicAllocation.initialExecutors=25 --driver-memory 7G --executor-memory 7G features_static-target_numNewOutlinks.py 

The parts in output can be merged with:
...
# crawl_final   = [7, 21, "07-21"]

crawl_initial = [9,  7, "09-07"] # integer tuples (month, day, pretty_string)
...
dataset = features \
	...
	.write.json("WebInsight/numNewOutlinks_dataset-between_"+crawl_initial[2]+"_and_"+crawl_final[2], mode='overwrite')
